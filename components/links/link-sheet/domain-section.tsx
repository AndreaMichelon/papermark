...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("dataroom.europastudios.it");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "dataroom.europastudios.it" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (isEditingCustomDomain && !canChangeCustomDomain) {
      setDisplayValue(data.domain ?? "dataroom.europastudios.it");
      return;
    }
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "dataroom.europastudios.it" });
      setDisplayValue("dataroom.europastudios.it");
      return;
    }

    // Check if this is a custom domain selection (not dataroom.europastudios.it)
    if (value !== "dataroom.europastudios.it") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "dataroom.europastudios.it")
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "dataroom.europastudios.it")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "dataroom.europastudios.it")
      : "dataroom.europastudios.it";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "dataroom.europastudios.it"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="dataroom.europastudios.it" className="hover:bg-muted">
              dataroom.europastudios.it
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "dataroom.europastudios.it" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "dataroom.europastudios.it" ? "flex" : "",
      ...
      {data.domain && data.domain !== "dataroom.europastudios.it" && !isDomainVerified ? (
